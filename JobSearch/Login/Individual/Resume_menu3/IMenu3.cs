...
namespace Login
{
    public partial class IMenu3 : UserControl
    {
        ...
        private void Resume_Insert()
        {
            ...
            {
                sqlconn.Open();
                string InsertText = "Insert Into RESUME (RE_NUM,ID,NAME,ADDR,EMAIL,PHONE,RE_SUBJECT,LICENSE,EXP,CONTENT,LOCATION) " 
                    + "VALUES(NEXT VALUE FOR RE_NUMBER,@id,@name,@addr,@email,@phone,@re_subject,@license,@exp,@content,@location)";
